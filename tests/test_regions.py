...
def run_sections(arch, filename, sections):
    ...
    assert len(ld.main_object.sections) == len(sections)
    for i, section in enumerate(ld.main_object.sections):
        assert section.name == sections[i].name
        assert section.offset == sections[i].offset
        assert AT.from_mva(section.vaddr, ld.main_object).to_lva() == sections[i].vaddr
        assert section.memsize == sections[i].memsize

    # address lookups
    assert ld.main_object.sections.find_region_containing(-1) is None
    ...
    for i in range(len(mapped_sections) - 1):
        ...
        if sec_a.vaddr + sec_a.memsize < sec_b.vaddr:
            ...
            for j in range(min(sec_b.vaddr - (sec_a.vaddr + sec_a.memsize), 20)):
                ...
                assert ld.main_object.find_section_containing(a) is None
                assert ld.main_object.sections.find_region_containing(a) is None

    assert ld.main_object.find_section_containing(0xFFFFFFFF) is None
...
def run_segments(arch, filename, segments):
    ...
    assert len(ld.main_object.segments) == len(segments)
    for i, segment in enumerate(ld.main_object.segments):
        assert segment.offset == segments[i].offset
        assert segment.vaddr == segments[i].vaddr
        assert segment.memsize == segments[i].memsize
        assert segment.filesize == segments[i].filesize

    # address lookups
    assert ld.main_object.segments.find_region_containing(-1) is None
    ...
    for segment in mapped_segments:
        assert (
            ld.main_object.find_segment_containing(segment.vaddr).vaddr == segment.vaddr
        )
        assert (
    ...
    for i in range(len(mapped_segments) - 1):
        ...
        if seg_a.vaddr + seg_a.memsize < seg_b.vaddr:
            ...
            for j in range(min(seg_b.vaddr - (seg_a.vaddr + seg_a.memsize), 20)):
                ...
                assert ld.main_object.find_segment_containing(a) is None
                assert ld.main_object.segments.find_region_containing(a) is None

    assert ld.main_object.find_segment_containing(0xFFFFFFFF) is None
